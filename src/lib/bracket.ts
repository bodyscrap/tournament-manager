...
// =====================================================
// Auto-advance byes
// If a match has one player and one bye (null),
// the real player wins automatically
// =====================================================
export function autoAdvanceByes(matches: Match[]): void {
  const matchMap = new Map<string, Match>(matches.map((m) => [m.id, m]));

  // Map incoming feeder matches for each slot so we can distinguish
  // true BYEs from not-yet-decided upstream participants.
  const incomingBySlot = new Map<string, { slot1: boolean; slot2: boolean }>();
  for (const m of matches) {
    if (m.next_match_id && m.next_match_slot) {
      const incoming = incomingBySlot.get(m.next_match_id) ?? {
        slot1: false,
        slot2: false,
      };
    ...
    if (m.loser_next_match_id && m.loser_next_match_slot) {
      const incoming = incomingBySlot.get(m.loser_next_match_id) ?? {
        slot1: false,
        slot2: false,
      };
  ...
  while (changed) {
    ...
    for (const m of matches) {
      ...
      // If both are null, skip (placeholder)
      if (p1 === null && p2 === null) continue;
      // If one is bye
      if (p1 !== null && p2 === null) {
        // Slot 2 has an upstream feeder -> participant undecided, not BYE.
        if (incoming.slot2) continue;
        m.winner_id = p1;
        m.status = "completed";
        advanceWinner(m, p1, matchMap);
        changed = true;
      } else if (p1 === null && p2 !== null) {
        // Slot 1 has an upstream feeder -> participant undecided, not BYE.
        if (incoming.slot1) continue;
        m.winner_id = p2;
        m.status = "completed";
        advanceWinner(m, p2, matchMap);
        changed = true;
      }
    }
  }
